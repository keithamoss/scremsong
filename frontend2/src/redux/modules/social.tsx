...
import { IReviewerAssignment } from './interfaces.reviewers'
...
export default function reducer(state: IModule = initialState, action: IAction) {
  switch (action.type) {
    ...
    case WS_REVIEWERS_LIST_ASSIGNMENTS:
      Object.values(action.assignments).forEach((assignment: IReviewerAssignment, _index: number) => {
...
export const getTweetsForAssignment = createSelector([getTweetAssignments, getTweets], (tweetAssignments, tweets) =>
...
export const getTweetAssignmentsForColumn = createSelector([getTweetAssignments], (tweetAssignments) =>
...
export const getTweetsForColumn = createSelector([getTweetAssignments, getTweets], (_tweetAssignments, tweets) =>
  memoize((columnTweetIds: string[]) => {
...
export const getUnreadTweetIds = memoize(
  ...
  (assignment: IReviewerAssignment, tweets: ISocialTweetList) =>
    `${assignment.id}.${assignment.last_read_on}.${tweets.length}`
)
...
export const isTweetUnread = memoize(
  ...
  (assignment: IReviewerAssignment, tweet: ISocialTweet) =>
    `${assignment.id}.${tweet.data.id_str}.${assignment.last_read_on}`
)
...
export function fetchTweets(startIndex: number, stopIndex: number, columns: number[] = []) {
  ...
  return async (dispatch: Function, _getState: Function, { api }: IThunkExtras) => {
    ...
    await dispatch(loadTweets(json as ISocialTweetsAndColumnsResponse))
    await dispatch(triageLoadTweets(json as ISocialTweetsAndColumnsResponse))
  }
}

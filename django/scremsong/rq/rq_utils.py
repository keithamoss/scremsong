...
from scremsong.app.enums import TaskStatus
...
def get_job_name_from_meta_or_none(job):
    if job.meta is not None and "_scremsong_job_name" in job.meta:
        return job.meta["_scremsong_job_name"]
    return None
...
def is_job_started(scremsong_job_name, queue_name=None, except_for_job_id=None, return_duplicate_jobs=False):
    duplicate_jobs = []
    redis = get_redis_connection()
    for queue in Queue.all(connection=redis):
        if queue_name is None or queue.name == queue_name:
            ...
            for job_id in registry.get_job_ids():
                ...
                if job.meta is not None and "_scremsong_job_name" in job.meta and job.meta["_scremsong_job_name"] == scremsong_job_name:
                    duplicate_jobs.append(job)

    return duplicate_jobs if return_duplicate_jobs is True else len(duplicate_jobs) > 0


# def is_job_started_debugging(scremsong_job_name, queue_name=None, except_for_job_id=None, return_duplicate_jobs=False):
#     logger.info(f"Hi from is_job_started for {scremsong_job_name} except_for_job_id {except_for_job_id}")
...
#                 job = Job.fetch(job_id, connection=redis)
#                 logger.info(f"_scremsong_job_name is {job.meta['_scremsong_job_name']} vs {scremsong_job_name}")

#                 if job.meta is not None and "_scremsong_job_name" in job.meta and job.meta["_scremsong_job_name"] == scremsong_job_name:
#                     duplicate_jobs.append(job)

#     logger.info(f"is_job_started duplicate_jobs for {scremsong_job_name} is len = {len(duplicate_jobs)}")
#     return duplicate_jobs if return_duplicate_jobs is True else len(duplicate_jobs) > 0


def is_job_queued(scremsong_job_name, queue_name=None, except_for_job_id=None, return_duplicate_jobs=False):
    duplicate_jobs = []
    redis = get_redis_connection()
    for queue in Queue.all(connection=redis):
        if queue_name is None or queue.name == queue_name:
...
def is_job_started_or_queued(scremsong_job_name, queue_name=None, except_for_job_id=None):
    if is_job_started(scremsong_job_name, queue_name, except_for_job_id) is True:
        return True
    if is_job_queued(scremsong_job_name, queue_name, except_for_job_id) is True:
        return True
    logger.info(f"Job '{scremsong_job_name}' is not started or queued")
    return False


def get_duplicate_jobs(job, queue_name=None):
    scremsong_job_name = get_job_name_from_meta_or_none(job)
    return is_job_started(scremsong_job_name, queue_name=queue_name, except_for_job_id=job.id, return_duplicate_jobs=True) + is_job_queued(scremsong_job_name, queue_name=queue_name, except_for_job_id=job.id, return_duplicate_jobs=True)

    # if scremsong_job_name == "task_open_tweet_stream":
    #     return is_job_started_debugging(scremsong_job_name, queue_name=queue_name, except_for_job_id=job.id, return_duplicate_jobs=True) + is_job_queued(scremsong_job_name, queue_name=queue_name, except_for_job_id=job.id, return_duplicate_jobs=True)
    # else:
    #     return is_job_started(scremsong_job_name, queue_name=queue_name, except_for_job_id=job.id, return_duplicate_jobs=True) + is_job_queued(scremsong_job_name, queue_name=queue_name, except_for_job_id=job.id, return_duplicate_jobs=True)
...
def cancel_jobs(jobs):
    ...
    for job in jobs:
        logger.info(f"Cancelling job '{job.id}' (_scremsong_job_name={get_job_name_from_meta_or_none(job)}) (status={job.get_status()}) on queue '{job.origin}'")
...
def log_task_debug_info(job):
    ...
    already_exists_elsewhere = is_job_started_or_queued(job.meta["_scremsong_job_name"], except_for_job_id=job.id)
